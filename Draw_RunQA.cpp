...
void Draw_RunQA(std::string inputData, bool savePlots = true, bool saveRoot = true)
{
  // Making input
  // ==========================
  ...
  const int canvasWidth = nChips*padWidth;
  const int canvasHeight = static_cast<int>(padWidth*(1.0-leftMargin-rightMargin)/(1.0-topMargin-bottomMargin));
  ...
  for (int ipad=0; ipad<10; ipad++) {
    ...
    double xMin = static_cast<double>(ipad)/nChips;
    double xMax = static_cast<double>(ipad+1)/nChips;

    TPad* p0 = new TPad(Form("p%d_c0", ipad), Form("p%d_c0", ipad), xMin, 0.0, xMax, 1.0);
    TPad* p1 = new TPad(Form("p%d_c1", ipad), Form("p%d_c1", ipad), xMin, 0.0, xMax, 1.0);
  ...
  // Draw eff per chip
  c2 -> cd();
  TPad* p2 = new TPad("p3", "p3", 0.0, 0.0, 0.5, 1.0);
